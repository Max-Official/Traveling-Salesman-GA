...
- Generate 50 routes, each route is an individual, all ind make up the population.
- Calcualte metrics for all individuals, aggregrate metrics for the first iteration (this iteration).
...
# generate a population with num of inidviduals
...
def main():
    ...
    cwd = os.path.dirname(os.path.abspath(__file__))    # get dir of this particular GA_TS.py file, whereever it is running
    ...
    best_indiv = random.sample(population, 1)[0]    # IMPORTANT to use [0], because random.sample returns a list of list here.
